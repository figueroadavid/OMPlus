function Remove-OMPlusEPRRecord {
    ...
    begin {
        <#
            Write-Verbose -Message 'Store the original WhatIf setting; turning it off temporarily'
            $OriginalWhatIf   = $WhatIfPreference
            $WhatIfPreference = $false
        #>

        $EPSPath    = [system.io.path]::Combine($Global:OMPLusSystemPath, 'eps_map')
        $BackupPath = [system.io.path]::Combine($Global:OMPLusSystemPath, ('eps_map_{0}.bkp' -f [datetime]::Now.ToString('yyyyMMdd_hhmmss')))

        $BackupFiles = Get-ChildItem -path $Global:OMPLusSystemPath -Filter "eps_map*.bkp"
        ...
        if ($BackupCount -ge 10 ) {
            ...
            <#
                Turn WhatIf back on if it was there to keep it from deleting the old backups
                When WhatIf is not used, they will be cleaned up properly
            #>
        ...
        Write-Verbose -Message ('Creating backup of the eps_map ({0})' -f $BackupPath) -Verbose
        try {
        ...
        Write-Verbose -Message ('Importing the eps_map into a usable format; this will take a few seconds') -Verbose
        ...
        Write-Verbose -Message ('Import Complete') -Verbose

        Write-Verbose -Message 'Restoring the WhatIf state'
        $WhatIfPreference = $OriginalWhatIf
    }
