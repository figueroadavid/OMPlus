function Set-OMPlusPrinter {
    ...
    .PARAMETER PrinterName
        This is the name of the printer to modify
    .PARAMETER IPAddress
        This is the new ip address of the printer.  The script was built around IPv4, but
        there is nothing in the script to prevent it from using IPv6 (if lpadmin.exe supports it)
    .PARAMETER Port
        This is the IP address to use for printing on the newly created printer.
    ...
    param(
        ...
        [parameter(ValueFromPipelineByPropertyName)]
        [ValidateSet('Beaker','WAM', 'Cognitive')]
        [string]$Comment,
    ...
    Begin {
        if ($IsTesting) { $PSBoundParameters }
    ...
    process {
        ...
        foreach ($Parameter in $PSBoundParameters.Keys) {
            switch ($Parameter) {
                ...
                'DriverType'        { $null = $ArgString.Add(  '-oPT{0}' -f ($DriverType -join ',' ));          break}
                'Mode'              { $null = $ArgString.Add( ('-omode="{0}"' -f $Mode));                       break}
        ...
        $LPAdmin = ('{0}\bin\lpadmin.exe' -f (Get-ItemProperty -Path HKLM:\SOFTWARE\PlusTechnologies\OMPlusServer -Name omhomepath).omhomepath )
        if ($IsTesting) {
            '{0} {1}' -f $LPAdmin, ($ArgString -join ' ')
        }
        else {
            Start-Process -FilePath $LPAdmin -ArgumentList ($ArgString -join ' ') -Wait -Verb RunAs
        }
    }
}
